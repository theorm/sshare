...
// echo "secret" | openssl enc -des3 -base64
...
// echo "123asdf" | openssl enc -d -des3 -base64
...
Secrets.prototype.split = function(message, numberOfShares, threshold, cb) {
  try {
    ...
    var shares = [];
    if (numberOfShares < 2 || threshold < 2) {
      for(var i = 0; i < numberOfShares; i++) {
        shares.push(hexMessage);
      }
    } else {
      shares = secrets.share(hexMessage, numberOfShares, threshold);      
    }
    cb(null, shares);
  } catch(e) {
    cb(e);
...
Secrets.prototype.combine = function(shares, cb) {
  try {
    ...
    if (shares.length < 2) {
      ...
      hexMessage = secrets.combine(shares);
    }
